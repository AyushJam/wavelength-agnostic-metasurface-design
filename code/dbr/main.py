...
# DBR specific
epSbS = refractive_indices["Sb2S3"] ** 2
epSiO = refractive_indices["SiO2"] ** 2

# define DBR layer thickness
thick_sbs = CENTER_WAV / (4 * np.sqrt(epSbS))
thick_sio = CENTER_WAV / (4 * np.sqrt(epSiO))
...
def objective_function(params, Nx=200, Ny=200):
  ...
  L1 = [l, 0]
  L2 = [0, l]
  pthick = [absorber_thickness, h] + [thick_sio, thick_sbs] * 4
  ...
  R, T = [], []
  for f in f_sampled:
    R1, T1, _ = get_rt_nk(L1, L2, pthick, f, epgrid2, epgrid3, epgrid4, Nx, Ny)
    R.append(R1)
    T.append(T1)
...
save_spectrum_data(
    xopt, f_spectrum, N_HOLES, ep1_diel, epbkg, epSbS, epSiO, thick_sbs, thick_sio, Nx, Ny
)
