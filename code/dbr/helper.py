...
from config import absorber_material, absorber_thickness
...
def get_epgrid_nk(f, epgrid2, epgrid3, epgrid4, Nx, Ny):
    """
    ...
    # Combine permittivity grids for alternating dbr layers
    epgrid_bilayer = list(np.concatenate((epgrid3.flatten(), epgrid4.flatten()))) * 4
    epgrid_bilayer = np.array(epgrid_bilayer)

    epgrid = np.concatenate((epgrid_1.flatten(), epgrid2.flatten(), epgrid_bilayer))
    return epgrid


def get_rt_nk(L1, L2, pthick, f, epgrid2, epgrid3, epgrid4, Nx, Ny):
    """
    ...
    nG = 101
    theta = 0.0  # Angle of incidence (normal incidence)
    ...
    obj.GridLayer_geteps(get_epgrid_nk(f, epgrid2, epgrid3, epgrid4, Nx, Ny))
...
def save_spectrum_data(
    ...
    L1 = [xopt[0], 0]
    L2 = [0, xopt[0]]
    pthick = [absorber_thickness, xopt[1]] + [thick_sio, thick_sbs] * 4
    ...
    epgrid3 = np.ones((Nx, Ny)) * epSiO
    epgrid4 = np.ones((Nx, Ny)) * epSbS

    absorptivity = []
    transmission = []

    for f in spectrum:
        R, T, obj = get_rt_nk(L1, L2, pthick, f, epgrid2, epgrid3, epgrid4, Nx, Ny)
    ...
    np.savetxt(spectrum_filename, spectrum)
...
def plot_absorption_spectrum(
    spectrum_filename="spectrum.txt", abs_filename="absorption.txt", save_fig=True
):
    """
    ...
    fig, ax = plt.subplots()
    ax = sns.lineplot(x=1 / spectrum, y=absorption)
